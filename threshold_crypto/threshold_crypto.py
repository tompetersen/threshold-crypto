...
class KeyParameters:
    ...
    @staticmethod
    def from_dict(obj: dict):
        return KeyParameters(obj['p'], obj['q'], obj['g'], obj['zq'])

    def __init__(self, p: int, q: int, g: int, zq: list):
        """
        ...
        :param zq: group Z_q* generated by g
        """
        if (2 * q + 1) != p:
    ...
    def __str__(self):
        zs = ["%d*x^%d " % (z, i) for (i, z) in enumerate(self._zq)]
        return 'KeyParameters:\n\tp = %d\n\tq = %d\n\tg = %d\n\tzq = %s' % (self._p, self._q, self._g, " ".join(zs))
...
class ThresholdCrypto:
    ...
    def static_512_key_parameters() -> KeyParameters:
        ...
        return KeyParameters(p=p, q=q, g=g, zq=[])
    ...
    def static_manual_key_parameters():
        ...
        return KeyParameters(p=p, q=q, g=g, zq=zq)
